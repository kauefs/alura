# <p align=center><font color=#F0F0F0 font-family=Georgia><ins>ƊⱭȾɅViƧi🧿Ƞ</ins><b>&trade;</b></font></p>

<div align=center>

[![License ](https://img.shields.io/badge/Apache--2.0-D22128?style=flat&logo=apache&logoColor=CB2138&label=License&labelColor=6D6E71)](https://www.apache.org/licenses/LICENSE-2.0)

[![GitHub  ](https://img.shields.io/badge/-000000?logo=github&logoColor=FFFFFF)](https://github.com/kauefs/)
[![Medium  ](https://img.shields.io/badge/-000000?logo=medium&logoColor=FFFFFF)](https://medium.com/@kauefs)
[![LinkedIn](https://img.shields.io/badge/in-0077B5?logo=linkedin&logoColor=FFFFFF)](https://www.linkedin.com/in/kauefs/)
[![Python  ](https://img.shields.io/badge/3-646464?logo=python&logoColor=FFDE57&labelColor=4584B6)](https://www.python.org/)

</div>

$$31\ January\ 2025$$

## Font-End Immersion $2^{nd}$ Edition

HTML **&** CSS **&** JavaScript **&** ReAct

[SpotiFy](https://spotyfi.vercel.app/)

$$22\ November\ 2024$$

## Back-End Immersion

Back-End DevelopMent with JavaScript

[InstaByte](https://picpostme.vercel.app/)

$$7\ September\ 2024$$

## Dev with [Google Gemini](https://gemini.google.com/) Immersion

Web Page **&** Data Base using JavaScript.

[WebSite](https://vermeer.vercel.app/)

$$10\ May\ 2024$$

## IA [Alura](https://www.alura.com.br/) **+** [Google](https://gemini.google.com/) Immersion $2^{nd}$ Edition

### <ins>Pesquisa na WikipédiA</ins>

Projeto inspirado em [exemplo](https://github.com/google-gemini/cookbook/blob/main/examples/Search_reranking_using_embeddings.ipynb) produzido pelo Google, usando funcionalidade _Embedding_ da Inteligência Artificial (IA) **Gemini** do Google para ranquear resultados de busca na WikipédiA.

_Embedding_**:** técnica de Processamento de Linguagem Natural (PLN) que converte texto em vetores numéricos, capturando significado semântico **&** contexto, de forma que textos com conteúdos semelhantes apresentam _embeddings_ mais próximos, permitindo comparações textuais **&** relacionamento entre textos, facilitando busca **&** classificação.

[WebApp](https://searchb0t.streamlit.app/)

##

<div align=center>

[![ƊⱭȾɅViƧi🧿Ƞ](https://img.shields.io/badge/ƊⱭȾɅViƧi&#x1F9FF;Ƞ&trade;-0065FF?style=plastic&label=&copy;2025&labelColor=0065FF)](https://datavision.one/)

</div>
