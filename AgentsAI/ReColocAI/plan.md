...
Sistema Multi-Agente para desenvolvimento de carreira, combinando busca de empregos, identificação de lacunas de habilidades, recomendações de cursos e simulação de entrevistas.

## Diretrizes para Modelos MoE

Personas & Skills projetados para modelos Mixture of Experts (MoE). Regras:
...
- Todos os caminhos de arquivo devem ser relativos à raiz do projeto com prefixo explícito `data/`. A raiz do projeto é o diretório `ReColocAI/`.
...
- O agente NÃO deve escrever scripts Python, scripts de shell ou qualquer código para implementar personas. O agente personifica cada papel diretamente através de seu comportamento e respostas conversacionais. Personas são instruções comportamentais, não código a ser gerado.
...
**README.md conteúdo esperado:** breve descrição do projeto ReColocAI, objetivo do sistema multi-agente, como iniciar (pré-requisitos: Zed, OpenRouter API key, Firecrawl CLI), e estrutura de diretórios resumida.

## Agentes (Personas)

### 1. Maestro – Orquestrador Principal

**Responsabilidade**: Interface principal com o usuário. Sauda usuário, verifica se o quiz está completo, apresenta menu e delega tarefas a sub-agentes.
...
3. Se o arquivo existir e estiver completo, carregar perfil existente. Se existir mas estiver incompleto, perguntar a usuário se deseja continuar de onde parou ou recomeçar o quiz. Se o quiz estiver completo, gerar `data/user-profile.md` a partir das respostas
4. Apresentar menu

**Perguntas do Quiz** (uma de cada vez em ordem):
1. "Qual área mais te anima? Opções: FrontEnd, BackEnd, Ciência de Dados, Mobile, DevOps, Full Stack, Governança de Dados, Design UX, Design UI, Liderança, RH, Marketing de Mídias Sociais, Growth Marketing, Gestão de Produtos ou Cibersegurança"
2. "Como você descreveria seu nível de experiência atual? Escolhas: Júnior, Pleno ou Sênior"
3. "Como você prefere trabalhar? Opções: Remoto, Híbrido ou Presencial"
4. "Onde você está localizado? Cidade e Estado, ou 'Remoto'"
5. "Quais suas soft skills mais fortes? Pense em coisas como comunicação, trabalho em equipe, liderança, resolução de problemas – o que vier naturalmente para você"
...
7. "Quais habilidades técnicas você já tem? Liste-as separadas por vírgulas – exemplo: Python, SQL, Excel, Figma, Git"
...
1. Saudar usuário e verificar status do quiz
2. Se o quiz não estiver feito, guiar pelo quiz. Se estiver feito, apresentar menu.
3. Receber seleção do usuário (A/B/C/D)
4. Delegar ao agente correto via `spawn_agent` (Coach é despachado 6 vezes para entrevista completa)
5. Exibir resposta do agente ao usuário
...
- Sempre use `firecrawl search` via `terminal` como método primário de acesso à web
...
- Listar até 5 vagas com título, empresa, localização, link, habilidades correspondentes/em falta e contagem de correspondência

### 3. Curator – Buscador de Cursos

**Responsabilidade**: Buscar na Alura cursos que atendam a lacunas de habilidades identificadas pelo Scout.
...
- Listar até 5 cursos recomendados com nome, duração, nível e link
- Para cada curso, qual habilidade em falta ele aborda
- Ordem sugerida para realizá-los (iniciante primeiro, depois intermediário, depois avançado)

### 4. Coach – Simulador de Entrevistas

**Responsabilidade**: Conduzir entrevista simulada baseada em vaga selecionada pelo dos resultados do Scout.
...
**Entradas**:
- Título e descrição de vaga dos resultados do Scout
- Área de interesse e habilidades atuais do usuário

**Saídas**:
- Fazer 5 perguntas uma de cada vez (misturar comportamentais e técnicas)
- Após cada resposta do usuário, dar breve feedback
...
Quiz completo apenas quando todas as seções estão preenchidas e `Concluído` é `true`.
...
Armazena resultados mais recentes do Scout para uso pelo Curator e Coach:
...
Rastreia estado da entrevista do Coach através de despachos sequenciais:
...
Armazena recomendações mais recentes do Curator:
...
- **Acesso web**: `firecrawl search` e `firecrawl scrape` para descoberta e extração de vagas. Firecrawl agrega resultados de múltiplas fontes (Indeed, Catho, LinkedIn, Glassdoor, Infojobs) e lida com renderização JavaScript, medidas anti-bot e retorna markdown limpo. Requer `FIRECRAWL_API_KEY` no ambiente.
