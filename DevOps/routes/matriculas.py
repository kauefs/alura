...
from models         import Matricula as ModelMatricula, Aluno as ModelAluno, Curso as ModelCurso # Importing Models
...
def create_matricula(matricula:Matricula, db:Session=Depends(get_db)):
    ...
    if db_aluno is None or db_curso is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Aluno ou Curso não encontrado.')
...
def read_matriculas_por_nome_aluno(nome_aluno:str, db:Session=Depends(get_db)):
    ...
    for matricula in db_aluno.matriculas:
        ...
        if curso: cursos_matriculados.append(curso.nome)
    if not cursos_matriculados: raise HTTPException(status_code=404, detail=f'Aluno "{nome_aluno}" não possui matrículas cadastradas.')
...
def read_alunos_matriculados_por_codigo_curso(codigo_curso:str, db:Session=Depends(get_db)):
    ...
    if not db_curso: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Curso não encontrado.')
    alunos_matriculados=[ ]
    # Itera pelas matrículas do curso:
    for matricula in db_curso.matriculas:
        # Acessa o aluno diretamente pelo relacionamento:
        aluno=matricula.aluno
        # Verifica se o aluno existe (pode ter sido excluído):
        if aluno: alunos_matriculados.append(aluno.nome)
    if not        alunos_matriculados: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Nenhum aluno matriculado no curso "{db_curso.nome}".')
